...
void launch_(amp::accelerator_view & acc_view, int threads, int local_threads, F && f, const Args &... args)
{
...
{
    launch_(acc_view, threads, local_threads, std::forward<F>(f), make_value(args)...);
}
...
void destruct(amp::accelerator_view & acc_view, int threads, wrapper<T> pointer)
{
    launch(acc_view, threads,5, 
        [](const int idx, const wrapper_val<T>& p) [[hc]] {
            (p.buffer)[ idx ].~T();
        }, pointer);
}
...
{
    launch(acc_view, threads, 1,
        [](const int idx, const wrapper_val<T>& p, Args const &... args) [[hc]] {
